...
import 'package:growk_v2/features/wallet_page/provider/load_wallet_amount_notifier.dart';
...
import 'package:growk_v2/features/wallet_page/use_case/get_wallet_balance_use_case.dart';
...
final walletBalanceProvider = FutureProvider<GetWalletBalanceModel>((ref) async {
  final useCase = ref.watch(getWalletBalanceUseCaseProvider);
  return useCase(); // calls the Future<GetWalletBalanceModel> call()
});

//fetching provider
final getNewWalletBalanceProvider = FutureProvider<GetWalletBalanceModel>((ref) async {
...
final loadWalletAmountUseCaseProvider = Provider<LoadWalletAmountUseCase>((ref) {
  final repo = ref.read(loadWalletAmountRepoProvider);
  return LoadWalletAmountUseCase(repo);
});

final loadWalletAmountNotifierProvider = StateNotifierProvider<LoadWalletAmountNotifier, LoadWalletAmountState>((ref) {
